...
import { Play, Code, CheckCircle, AlertTriangle, FileCode2, Terminal as TerminalIcon, Sparkles, Brain, Send, Check } from 'lucide-react';
import axios from 'axios';

function App() {
  const { files, activeFile, setActiveFile, updateFile, addFile } = useIdeStore();
  ...
  const [chatMessages, setChatMessages] = useState<{role: 'user' | 'agent', text: string, code?: string, action?: string, targetFile?: string}[]>([]);
  ...
  const handleApplyAction = (action: string, targetFile: string, code: string) => {
                                ...
                                <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                                    ...
                                    {msg.code && msg.role === 'agent' && (
                                        <div className="mt-2 w-full max-w-[95%] bg-[#0A0516] border border-fuchsia-900/50 rounded-xl overflow-hidden shadow-lg">
                                            ...
                                            <div className="p-2 border-t border-fuchsia-900/30 bg-[#0F0822] flex justify-end">
                                                <button 
                                                    onClick={() => handleApplyAction(msg.action!, msg.targetFile!, msg.code!)}
